...
def main(args):
    g=load_dataset(args)
    g=g.to(args.gpu)
    model=GATConv_000(args.in_feats,args.out_feats,args.num_heads).to(args.gpu)
    ...
    print(maxMemory)
    print("gatconv_000 forward time:",(end-start)/args.epochs)

    with open('../figure8.csv','a') as f:
        print("gat_without_reorg,latency={}s,memory={}MB".format((end-start)/args.epochs,maxMemory),file=f)
