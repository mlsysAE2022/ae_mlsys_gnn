...
import GPUtil
sys.path.append('../..')
from layers.gatconv_layer import GATConv_no_fusion
...
def main(args):
    ...
    model=GATConv_no_fusion(args.in_feats,args.out_feats,args.num_heads).to(args.gpu)
    model.eval()
    features=torch.rand(row_ptr.shape[0]-1,args.in_feats,device=args.gpu)
    
    if args.profileio:
        profile_start()
        model(row_ptr,col_ind,col_ptr,row_ind,features,None,g)
        profile_end()
        exit()
    ...
    with open('../figure9.csv','a') as f:
        print("gat_without_fusion,latency={}s,memory={}MB".format((end-start)/args.epochs,maxMemory),file=f)
