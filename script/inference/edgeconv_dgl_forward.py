...
import sys
sys.path.append('../..')
from layers.edgeconv_layer import EdgeConv_dgl
import time
import numpy as np
import GPUtil
...
nng = KNNGraph(args.k)
model=EdgeConv_dgl(3,args.out_feat).to(dev)
g = nng(data).to(dev)
data=data.to(dev)
...
print(maxMemory)
print("edgeconv_dgl forward time:",(end-start)/args.epochs)

with open('../figure8.csv','a') as f:
    print("edgeconv_without_reorg,latency={}s,memory={}MB".format((end-start)/args.epochs,maxMemory),file=f)
