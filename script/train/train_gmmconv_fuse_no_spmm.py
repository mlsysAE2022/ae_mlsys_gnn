...
import sys
sys.path.append('../..')
from layers.gmmconv_layer import GMMConv_fuse_no_spmm
...
class MoNet(nn.Module):
    def __init__(self,
        ...
        # Input layer
        self.layers.append(
            GMMConv_fuse_no_spmm(in_feats, n_hidden, dim, n_kernels))
        self.pseudo_proj.append(
            nn.Sequential(nn.Linear(2, dim), nn.Tanh()))

        # Hidden layer
        for _ in range(n_layers - 1):
            self.layers.append(GMMConv_fuse_no_spmm(n_hidden, n_hidden, dim, n_kernels))
            self.pseudo_proj.append(
                nn.Sequential(nn.Linear(2, dim), nn.Tanh()))

        # Output layer
        self.layers.append(GMMConv_fuse_no_spmm(n_hidden, out_feats, dim, n_kernels))
        self.pseudo_proj.append(
            nn.Sequential(nn.Linear(2, dim), nn.Tanh()))
