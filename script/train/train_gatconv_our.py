...
class GAT(nn.Module):
    ...
    def forward(self,row_ptr,col_ind,col_ptr,row_ind,inputs,save_memory):
        h = inputs
        for l in range(self.num_layers):
            h = self.gat_layers[l](row_ptr,col_ind,col_ptr,row_ind,h,save_memory).flatten(1) # h.shape[-1] = num_heads*out_feats
        # output projection
        logits = self.gat_layers[-1](row_ptr,col_ind,col_ptr,row_ind,h,save_memory).mean(1)
        return logits
...
def main(args):
    ...
    with open("../{}".format(args.output),'a') as f:
        print("train_GAT_our,dataset={},head={},f={},latency={}s,memory={}MB".format(args.dataset,args.num_heads,args.num_hidden,train_time,maxMemory),file=f) 
