...
sys.path.append('../..')
import GPUtil
from layers.gmmconv_layer import GMMConv_stash
...
class MoNet(nn.Module):
    def __init__(self,
        ...
        # Input layer
        self.layers.append(
            GMMConv_stash(in_feats, n_hidden, dim, n_kernels))
        self.pseudo_proj.append(
            nn.Sequential(nn.Linear(2, dim), nn.Tanh()))

        # Hidden layer
        for _ in range(n_layers - 1):
            self.layers.append(GMMConv_stash(n_hidden, n_hidden, dim, n_kernels))
            self.pseudo_proj.append(
                nn.Sequential(nn.Linear(2, dim), nn.Tanh()))

        # Output layer
        self.layers.append(GMMConv_stash(n_hidden, out_feats, dim, n_kernels))
        self.pseudo_proj.append(
            nn.Sequential(nn.Linear(2, dim), nn.Tanh()))
...
def main(args):
    ...
    with open("../figure10.csv",'a') as f:
        print("train_monet_stash,dataset={},k={},d={},latency={}s,memory={}MB".format(args.dataset,args.n_kernels,args.pseudo_dim,(t2 - t1)/args.n_epochs,maxMemory),file=f) 
