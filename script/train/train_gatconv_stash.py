...
from layers.gatconv_layer import GATConv_stash
...
class GAT(nn.Module):
    def __init__(self,
        ...
        self.gat_layers.append(GATConv_stash(
    ...
    def forward(self,row_ptr,col_ind,col_ptr,row_ind,inputs,permute):
        h = inputs
        for l in range(self.num_layers):
            h = self.gat_layers[l](row_ptr,col_ind,col_ptr,row_ind,h,permute).flatten(1) # h.shape[-1] = num_heads*out_feats
        # output projection
        logits = self.gat_layers[-1](row_ptr,col_ind,col_ptr,row_ind,h,permute).mean(1)
        return logits
...
def main(args):
    ...
    for _ in range(10):
        model.train()
        logits = model(row_ptr,col_ind,col_ptr,row_ind,features,permute)
    ...
    model.eval()
    with torch.no_grad():
        logits = model(row_ptr,col_ind,col_ptr,row_ind,features,True)
    ...
    with open("../figure10.csv",'a') as f:
        print("train_GAT_stash,dataset={},head={},f={},latency={}s,memory={}MB".format(args.dataset,args.num_heads,args.num_hidden,train_time,maxMemory),file=f) 
