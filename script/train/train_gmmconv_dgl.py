...
class MoNet(nn.Module):
    def __init__(self,
        ...
        # Input layer
        self.layers.append(
            GMMConv_dgl(in_feats, n_hidden, dim, n_kernels))
        self.pseudo_proj.append(
            nn.Sequential(nn.Linear(2, dim), nn.Tanh()))

        # Hidden layer
        for _ in range(n_layers - 1):
            self.layers.append(GMMConv_dgl(n_hidden, n_hidden, dim, n_kernels))
            self.pseudo_proj.append(
                nn.Sequential(nn.Linear(2, dim), nn.Tanh()))

        # Output layer
        self.layers.append(GMMConv_dgl(n_hidden, out_feats, dim, n_kernels))
        self.pseudo_proj.append(
            nn.Sequential(nn.Linear(2, dim), nn.Tanh()))
...
def main(args):
    ...
    print("max memory {:.4f} MB".format(maxMemory))
    ...
    torch.cuda.synchronize()
    t2 = time.time()
    train_time=(t2 - t1)/args.n_epochs
    ...
    with open("../{}".format(args.output),'a') as f:
        print("train_monet_{},dataset={},k={},d={},latency={}s,memory={}MB".format(args.key,args.dataset,args.n_kernels,args.pseudo_dim,train_time,maxMemory),file=f) 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--key",type=str,default="dgl")
