...
__global__ void fused_forward_kernel(int m, int nnz, int h, int f,
                                     ...
                                     float *edge_leakyrelu, float *edge_softmax,
                                     float *out_feat) {
  int rid = blockIdx.x;
  int hid = blockIdx.y;
  int lb = row_ptr[rid];
  ...
  for (int j = 0; j < loop; j++) {
    ...
    if (pid < hb) {
      ...
      edge_leakyrelu[pid*h+hid]=weight;
    }
    __syncwarp();
...
void gat_forward(int m, int nnz, int h, int f, const float *attn_row,
                 ...
                 float negative_slope, float *edge_leakyrelu, float *edge_softmax,
...
//   if (f > 64)
    fused_forward_kernel<<<dim3(m, h, 1), dim3(32, (f + 31) / 32, 1),
        ...
        negative_slope, edge_leakyrelu, edge_softmax, out_feat);
//   else {
...
gat_forward_cuda(torch::Tensor attn_row, torch::Tensor attn_col,
  ...
  auto edge_leakyrelu = torch::empty({nnz, h}, options);
  auto edge_softmax = torch::empty({nnz, h}, options);
  gat_forward(m, nnz, h, f, attn_row.data_ptr<float>(),
              ...
              edge_leakyrelu.data_ptr<float>(), edge_softmax.data_ptr<float>(),
              in_feat.data_ptr<float>(), out_feat.data_ptr<float>());
  return {out_feat, edge_leakyrelu, edge_softmax};
}
...
//     // const int *permute,
//     const float *edge_leakyrelu, const float *edge_softmax, const float *attn_row,
...
__global__ void fused_backward_kernel(
    ...
    const float negative_slope, const float *edge_leakyrelu, const float *edge_softmax,
  ...
  for (int j = 0; j < loop; j++) {
    ...
    //   float val_leaky = LeakyRelu(attn_row_val + attn_col_val, negative_slope);
      ...
      float val_softmax =edge_softmax[pid*h+hid];
  ...
  for (int j = 0; j < loop; j++) {
    ...
    if (pid < hb) {
      int cid = col_ind[pid];
    //   float attn_col_val = attn_col[cid * h + hid];
      float val_leaky = edge_leakyrelu[pid*h+hid];//LeakyRelu(attn_row_val + attn_col_val, negative_slope);
      // if (val_leaky != edge_relu_csr[pid * h + hid])
      //     printf("leakyrelu value error\n");
      float val_softmax = edge_softmax[pid*h+hid];
          //exp(val_leaky - edge_max[rid * h + hid]) / edge_sum[rid * h + hid];
...
void gat_backward(int m, int nnz, int h, int f, float negative_slope,
                  ...
                  int *permute,
                  float *edge_leakyrelu, float *edge_softmax, float *in_feat,
...
mhspmm_backward_kernel<<<dim3(m, 1, 1), dim3(f, h, 1)>>>(
    m, nnz, h, f, col_ptr, row_ind, edge_softmax, grad, grad_feat,
    permute);
  ...
  fused_backward_kernel<<<dim3(m, 1, 1), dim3(32, h, 1)>>>(
      m, nnz, h, f, row_ptr, col_ind, negative_slope, edge_leakyrelu, edge_softmax,
...
std::vector<torch::Tensor> gat_backward_cuda(
  ...
  gat_backward(m, nnz, h, f, negative_slope, row_ptr.data_ptr<int>(),
               ...
               row_ind.data_ptr<int>(),permute.data_ptr<int>(), edge_max.data_ptr<float>(),
