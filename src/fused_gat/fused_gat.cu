...
__global__ void fused_scatter_kernel(int m, int nnz, int h, int f, const float *attn_row,
                                     const float *attn_col, const int *row_ptr,
                                     const int *col_ind, const float negative_slope,
                                     float *edge_relu_csr, float *edge_softmax_csr)
{
  int rid = blockIdx.x;
  int hid = threadIdx.y;
  int lb = row_ptr[rid];
  ...
  for (int j = 0; j < loop; j++)
  {
    ...
    if (pid < hb)
    {
      ...
      edge_relu_csr[pid * h + hid] = weight;
    }
    __syncwarp();
  ...
  for (int j = 0; j < loop; j++)
  {
    ...
    if (pid < hb)
    {
      exptmp = exp(edge_relu_csr[pid * h + hid] - weightMax);
    }
    __syncwarp();
    for (int stride = 16; stride > 0; stride >>= 1)
    {
  ...
  for (int j = 0; j < loop; j++)
  {
    ...
    if (pid < hb)
    {
      float weight = edge_relu_csr[pid * h + hid];
      weight = exp(weight - weightMax) / expAll;
      edge_softmax_csr[pid * h + hid] = weight;
    }
  }
}
...
void gat_forward(int m, int nnz, int h, int f, const float *attn_row,
                 ...
                 float negative_slope, float *edge_relu_csr,
                 float *edge_softmax_csr, const float *in_feat,
                 float *out_feat)
{
  // float rt;
  ...
  fused_scatter_kernel<<<dim3(m, 1, 1), dim3(32, h, 1)>>>(
      m, nnz, h, f, attn_row, attn_col, row_ptr, col_ind, negative_slope,
      edge_relu_csr, edge_softmax_csr);
  // cudaEventRecord(stop, 0);
  // cudaEventSynchronize(stop);
  // cudaEventElapsedTime(&rt, start, stop);
  // cout << "fused_scatter time:" << rt << endl;

  // cudaEventRecord(start, 0);
  if (h == 1)
    spmm_kernel(m, f, row_ptr, col_ind, edge_softmax_csr, in_feat, out_feat);
  else if (h * f <= 1024)
    mhspmm_kernel<<<dim3(m, 1, 1), dim3(32, h, 1)>>>(
        m, nnz, h, f, row_ptr, col_ind, edge_softmax_csr, in_feat, out_feat);
  else
  {
    printf("feature size too large\n");
    // mhspmm_kernel<<<dim3(m, 1, 1), dim3(256, h, 1)>>>(m, nnz, h, f, row_ptr,
    // col_ind, edge_softmax_csr, in_feat, out_feat);
  }
  // cudaEventRecord(stop, 0);
  // cudaEventSynchronize(stop);
  // cudaEventElapsedTime(&rt, start, stop);
  // cout << "forward mhspmm time:" << rt << endl;
}
                 ...
                 float negative_slope, torch::Tensor in_feat)
{
  ...
  gat_forward(
      ...
      edge_relu_csr.data_ptr<float>(), edge_softmax_csr.data_ptr<float>(),
      in_feat.data_ptr<float>(), out_feat.data_ptr<float>());
  return {out_feat, edge_relu_csr, edge_softmax_csr};
}

__global__ void mhspmm_backward_kernel(int m, int nnz, int h, int f,
                                       const int *row_ptr, const int *col_ind,
                                       const float *edge_softmax_csr /* E*H */,
                                       const float *grad_in /* V*H*F */,
                                       float *grad_out /* V*H*F */,
                                       const int *permute)
{
  int rid = blockIdx.x;
  int hid = threadIdx.y;
  ...
  for (int fid = threadIdx.x; fid < f; fid += blockDim.x)
  {
    ...
    {
      int offset1 = col_ind[ptr] * f * h + hid * f + fid;
      float att = edge_softmax_csr[permute[ptr] * h + hid];
      acc += att * grad_in[offset1];
    }
    grad_out[rid * h * f + hid * f + fid] = acc;
  }
}
...
__global__ void
fused_gather_row(int m, int nnz, int h, int f, const int *row_ptr,
                 const float negative_slope, const float *edge_softmax_csr,
                 const float *edge_relu_csr, const float *grad_edge_csr,
                 float *grad_edge_for_gather_csr, float *grad_attn_row)
{
  int rid = blockIdx.x;
  int hid = threadIdx.y;
  int lb = row_ptr[rid];
  ...
  for (int j = 0; j < loop; j++)
  {
    ...
    if (pid < hb)
    {
      weight = edge_softmax_csr[pid * h + hid] * grad_edge_csr[pid * h + hid];
    }
    __syncwarp();
    for (int stride = 16; stride > 0; stride >>= 1)
    {
  ...
  for (int j = 0; j < loop; j++)
  {
    ...
    if (pid < hb)
    {
      grad_out = edge_softmax_csr[eid] * (grad_edge_csr[eid] - weightSum);
      if (edge_relu_csr[eid] < 0)
...
void gat_backward(int m, int nnz, int h, int f, float negative_slope,
                  ...
                  int *permute, float *edge_softmax_csr, float *edge_relu_csr,
                  float *in_feat,
                  ...
                  float *grad_attn_col)            // output grad
{
  ...
  // cudaEventRecord(start, 0);
  if (h * f <= 1024)
    mhspmm_backward_kernel<<<dim3(m, 1, 1), dim3(f, h, 1)>>>(
        m, nnz, h, f, col_ptr, row_ind, edge_softmax_csr, grad, grad_feat,
        permute);
  else
    printf("feature size too large in backward\n");
  // cudaEventRecord(stop, 0);
  // cudaEventSynchronize(stop);
  // cudaEventElapsedTime(&rt, start, stop);
  // cout << "backward mhspmm time:" << rt << endl;
  ...
  // cudaEventSynchronize(stop);
  // cudaEventElapsedTime(&rt, start, stop);
  // cout << "backward mhsddmm time:" << rt << endl;

  // cudaEventRecord(start, 0);
  fused_gather_row<<<dim3(m, 1, 1), dim3(32, h, 1)>>>(
      m, nnz, h, f, row_ptr, negative_slope, edge_softmax_csr, edge_relu_csr,
      grad_edge_csr, grad_edge_for_gather_csr, grad_attn_row);
  // cudaEventRecord(stop, 0);
  // cudaEventSynchronize(stop);
  // cudaEventElapsedTime(&rt, start, stop);
  // cout << "fused gather row time:" << rt << endl;

  // cudaEventRecord(start, 0);
  gather_col<<<dim3(m, 1, 1), dim3(32, h, 1)>>>(
      m, nnz, h, f, col_ptr, permute, grad_edge_for_gather_csr, grad_attn_col);
  // cudaEventRecord(stop, 0);
  // cudaEventSynchronize(stop);
  // cudaEventElapsedTime(&rt, start, stop);
  // cout << "fused gather col time:" << rt << endl;
}

std::vector<torch::Tensor>
gat_backward_cuda(float negative_slope, torch::Tensor row_ptr,
                  torch::Tensor col_ind, torch::Tensor col_ptr,
                  torch::Tensor row_ind, torch::Tensor permute,
                  torch::Tensor edge_softmax_csr, torch::Tensor edge_relu_csr,
                  torch::Tensor in_feat, torch::Tensor grad)
{
  const auto m = row_ptr.size(0) - 1;
  const auto nnz = col_ind.size(0);
  const auto h = edge_softmax_csr.size(1);
  const auto f = in_feat.size(2);
  ...
  auto grad_attn_col = torch::empty({m, h}, options);
  // printf("gat backward cuda\n");
  gat_backward(
      m, nnz, h, f, negative_slope, row_ptr.data_ptr<int>(),
      col_ind.data_ptr<int>(), col_ptr.data_ptr<int>(), row_ind.data_ptr<int>(),
      permute.data_ptr<int>(), edge_softmax_csr.data_ptr<float>(),
      edge_relu_csr.data_ptr<float>(), in_feat.data_ptr<float>(),
      grad.data_ptr<float>(), grad_edge_csr.data_ptr<float>(),
      grad_edge_for_gather_csr.data_ptr<float>(), grad_feat.data_ptr<float>(),
